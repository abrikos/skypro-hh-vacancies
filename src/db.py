...
class DBManager:
    ...
    def prepare_db(self) -> None:
        with self._conn:
            create_companies_sql = ("CREATE TABLE IF NOT EXISTS employers("
                                    "employer_id serial primary key,"
                                    "name varchar(255)"
                                    ");")
            create_vacancies_sql = ("CREATE TABLE IF NOT EXISTS vacancies("
    ...
    def insert_vacancies(self, vacancies):
        sql_vacancies = "INSERT INTO vacancies (vacancy_id, name, employer_id, salary, url, city, requirement, responsibility) values "
        values = []
        for vac in vacancies:
            values.append(f"({vac["id"]} , '{vac["name"]}', {vac["employer"]}, {vac["salary"]}, '{vac["url"]}', '{vac["city"]}', '{vac["requirement"]}', '{vac["responsibility"]}')")
